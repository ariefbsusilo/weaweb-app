...
export default function RegisterPage() {
  return (
    <React.Suspense fallback={<div className="flex h-screen items-center justify-center"><Loader2 className="w-8 h-8 animate-spin" /></div>}>
      <RegisterContent />
    </React.Suspense>
  )
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ email, password, name, plan: planParam || "Starter" })
      })
    ...
    <div className="flex h-screen w-full bg-background overflow-hidden relative">
      ...
      <div className="relative z-10 w-full h-full flex items-center justify-center p-4">
        <motion.div 
          ...
          className="w-full max-w-5xl h-[650px] bg-card/40 backdrop-blur-2xl border border-border/50 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row"
        >
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-4 mb-12">
                <div className="w-16 h-16 flex items-center justify-center">
                  <img src="/logo.png" alt="Weaweb Logo" className="w-full h-full object-contain drop-shadow-md" />
                </div>
                <span className="text-4xl font-extrabold tracking-tight text-foreground">Weaweb</span>
              </div>
              
              <h1 className="text-4xl font-black tracking-tight leading-tight text-foreground mb-4">
                Start your <br/> automation <br/> <span className="text-primary">journey today.</span>
              </h1>
              <p className="text-lg text-muted-foreground font-medium max-w-sm">
                Join thousands of businesses scaling their WhatsApp marketing.
              </p>
            </div>
            
            <div className="relative z-10">
              <div className="flex items-center gap-4 text-sm font-semibold text-muted-foreground">
                <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-green-500" /> Plan Selected: {planParam || "None"}</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-card/60 relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <Label htmlFor="name" className="text-foreground/80 font-semibold">Workspace / Company Name</Label>
              ...
              <Button 
                type="submit" 
                disabled={loading}
                className="w-full h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-base rounded-xl shadow-[0_4px_14px_0_rgba(34,197,94,0.39)] hover:shadow-[0_6px_20px_rgba(34,197,94,0.23)] hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 group mt-4"
              >
